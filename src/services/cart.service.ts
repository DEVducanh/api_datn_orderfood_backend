...
export const createCartService = async (user_id: string | null, table_id: string) => {
  try {
    let cart
    if (user_id) {
...
export const addToCartService = async (user_id: string | null, table_id: string, dish_id: string, quantity: number) => {
  try {
    let cart
    if (user_id) {
      cart = await Cart.findOne({ user_id, table_id })
